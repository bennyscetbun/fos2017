			...
			<div class="panel-body">
				<form method="POST">
					...
					<div class="form-group {{with .errs}}{{with $errlist := index . $cpid}}has-error{{end}}{{end}}">
						<input type="text" class="form-control" name="confirm_{{.primaryID}}" placeholder="Confirmez {{title .primaryID}}" value="{{.confirmPrimaryIDValue}}" />
						{{with .errs}}{{with $errlist := index . $cpid}}{{range $errlist}}<span class="help-block">{{.}}</span>{{end}}{{end}}{{end}}
					</div>
					<input type="hidden" name="{{.xsrfName}}" value="{{.xsrfToken}}" />
					<div class="row">
						<div class="col-md-offset-1 col-md-10">
							<button class="btn btn-primary btn-block" type="submit">Envoyer l'email</button>
						</div>
					</div>
					<div class="row">
						<div class="col-md-offset-1 col-md-10">
