			...
			<div class="panel-body">
				<form method="POST">
					...
					<div class="form-group {{with .errs}}{{with $errlist := index . "password"}}has-error{{end}}{{end}}">
						<input type="password" class="form-control" name="password" placeholder="Mot de passe" value="{{.password}}" />
					...
					<div class="form-group {{with .errs}}{{with $errlist := index . "confirm_password"}}has-error{{end}}{{end}}">
						<input type="password" class="form-control" name="confirm_password" placeholder="Confirmez le mot de passe" value="{{.confirmPassword}}" />
					...
					<div class="row">
						<div class="col-md-offset-1 col-md-10">
							<a class="btn btn-link btn-block" href="{{mountpathed "login"}}">Annuler</a>
						</div>
					</div>
				</form>
			</div>
